...
run_lm_unpaired <- function(dat, antigen_name, overall_weights = "proportional") {
  ...
  if ("contrast" %in% names(out_s)) out_s$contrast <- as.character(out_s$contrast)
  ...
  ## ---------- B) Overall by cohort (Thailand/Brazil only), keep 3-way model ----------
  emm_str <- emmeans::emmeans(mA, ~ variant | cohort * category)
  summ_str <- summary(emm_str)
  emm_str <- emm_str[!is.na(summ_str$SE) & as.character(summ_str$cohort) != "Control"]
  emm_o <- emmeans::emmeans(emm_str, ~ variant | cohort, weights = "proportional")
  ct_o  <- emmeans::contrast(emm_o, "trt.vs.ctrl", ref = "Sal-1")
  out_o <- as.data.frame(summary(ct_o, infer = c(TRUE, TRUE), adjust = "BH"),
                         stringsAsFactors = FALSE)
  # harmonize possible CI names once
  if (all(c("LCL","UCL") %in% names(out_o))) {
    out_o <- dplyr::rename(out_o, lower.CL = LCL, upper.CL = UCL)
  } else if (all(c("asymp.LCL","asymp.UCL") %in% names(out_o))) {
    out_o <- dplyr::rename(out_o, lower.CL = asymp.LCL, upper.CL = asymp.UCL)
  }
  if ("contrast" %in% names(out_o)) out_o$contrast <- as.character(out_o$contrast)
  for (cn in c("estimate","SE","df","t.ratio","p.value","lower.CL","upper.CL")) {
  ...
  out_o$haplotype_vs <- sub(" - Sal-1$", "", out_o$contrast, fixed = TRUE)
  ...
  out_c$GM_ratio <- exp(out_c$estimate)
  out_c$GM_lo    <- exp(out_c$lower.CL)
  out_c$GM_hi    <- exp(out_c$upper.CL)
  out_c$antigen  <- antigen_name
  if ("contrast" %in% names(out_c)) out_c$contrast <- as.character(out_c$contrast)
  out_c <- dplyr::relocate(out_c, antigen, variant, category, contrast,
                           estimate, SE, df, lower.CL, upper.CL, t.ratio, p.value,
                           GM_ratio, GM_lo, GM_hi)
  
  ## ---------- D) Overall by cohort (Control only), keep 3-way model ----------
  emm_str_D <- emmeans::emmeans(mA, ~ variant | cohort * category)
  summ_D    <- summary(emm_str_D)
  # keep only Control strata and estimable cells
  emm_str_D <- emm_str_D[!is.na(summ_D$SE) & as.character(summ_D$cohort) == "Control"]
  # average over Control categories (TRC/BRC/ARC/VBDR)
  emm_D <- emmeans::emmeans(emm_str_D, ~ variant | cohort, weights = "proportional")
  ct_D  <- emmeans::contrast(emm_D, "trt.vs.ctrl", ref = "Sal-1")
  out_D <- as.data.frame(summary(ct_D, infer = c(TRUE, TRUE), adjust = "BH"),
                         stringsAsFactors = FALSE)
  if (all(c("LCL","UCL") %in% names(out_D))) {
    out_D <- dplyr::rename(out_D, lower.CL = LCL, upper.CL = UCL)
  } else if (all(c("asymp.LCL","asymp.UCL") %in% names(out_D))) {
    out_D <- dplyr::rename(out_D, lower.CL = asymp.LCL, upper.CL = asymp.UCL)
  }
  if ("contrast" %in% names(out_D)) out_D$contrast <- as.character(out_D$contrast)
  for (cn in c("estimate","SE","df","t.ratio","p.value","lower.CL","upper.CL")) {
    if (cn %in% names(out_D)) out_D[[cn]] <- numify(out_D[[cn]])
  }
  out_D <- dplyr::filter(out_D, !is.na(estimate), !is.na(lower.CL), !is.na(upper.CL))
  out_D$GMR         <- exp(out_D$estimate)
  out_D$GMR_lo      <- exp(out_D$lower.CL)
  out_D$GMR_hi      <- exp(out_D$upper.CL)
  out_D$antigen     <- antigen_name
  out_D$haplotype_vs <- sub(" - Sal-1$", "", out_D$contrast, fixed = TRUE)
  out_D <- dplyr::relocate(out_D, antigen, cohort, contrast, haplotype_vs,
                           estimate, SE, df, lower.CL, upper.CL, t.ratio, p.value,
                           GMR, GMR_lo, GMR_hi)
  
  list(
    detail             = out_s,
    overall_by_cohort  = out_o,       # TH/BR only
    country_comp       = out_c,
    overall_control    = out_D        # CONTROL only
  )
}

# ----------------------------
# Run, bind, and save outputs
# ----------------------------
...
overall_control_t <- dplyr::bind_rows(lapply(res, `[[`, "overall_control"))
...
if (nrow(overall_control_t)) overall_control_t <- overall_control_t %>% dplyr::group_by(antigen) %>% dplyr::mutate(q_BH = p.adjust(p.value, "BH")) %>% dplyr::ungroup()
...
if (nrow(overall_control_t)) write.csv(overall_control_t, "output/lmm/UNPAIRED_LM_OVERALL_CONTROL_ONLY.csv",       row.names = FALSE)

cat("Done. Files written (if non-empty):\n",
    "- output/lmm/UNPAIRED_LM_BY_STRATUM.csv\n",
    "- output/lmm/UNPAIRED_LM_OVERALL_BY_COHORT.csv  (Thailand/Brazil only)\n",
    "- output/lmm/UNPAIRED_LM_COUNTRY_PAIRWISE.csv\n",
    "- output/lmm/UNPAIRED_LM_OVERALL_CONTROL_ONLY.csv  (Control only)\n")
